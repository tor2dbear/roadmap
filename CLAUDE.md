...
- **`tor2dbear/roadmap`** — the workshop *and* one instance of the tool. The code,
  the 1245-check suite, this board's own config (`sources.json`,
...
- **The handle is a band; the pill is only its mark — because a margin belongs to no
  element.** `touch-action` is the only thing that keeps a gesture on our side of the line
  (`preventDefault` on a `pointermove` does not stop a pan), and `.sheet` itself has to stay
  `pan-y` or the list inside it could never scroll: the permitted behaviours are the
  *intersection* from the hit target up, so a `none` on the sheet would forbid `pan-y` on its
  own body. That leaves the chrome to cover its own pixels, and written the obvious way — a
  40×5 pill with `margin: 4px auto 14px` — it did not. Measured at 390px, the sheet's top
  30px: `.sheet` and `pan-y` for 0–9 and 16–29, the grip and `none` for **six**. Four fifths
  of the handle were the browser's, and where the browser takes a downward pan there is no
  scroller to give it to, so it goes to the page: reported from a phone as the whole page
  dragging down with iOS's own refresh spinner above it, and *"it works when I drag further
  down"* — which is `.sheet-head`, at 30px, and `none`. The band owns those pixels now (the
  sheet's 6px top padding and both margins are its padding), so the geometry is unchanged —
  pill 40×5, 11px down, head at 30 — and the target went from 40×5 to **374×29**. The pill is
  a `::before`, since a background on the band would paint the whole strip.
  The check walks the chrome pixel by pixel and asks the question *the browser* asks —
  intersecting from the hit target upwards — rather than reading one element's own value:
  `.sheet-title` computes `auto` and is perfectly safe under a head that says `none`, so the
  narrow reading would have cleared the pill and condemned the title, wrong on both counts.
...
- **`priority`, `agent`, `target` — the list is explicit, never "everything with a `has`".**
  ...
  a way in.
- **The mark is the ghost of the value, not one em dash for all of them.** The first
  version drew `\u2014` everywhere, and the board had chosen it already — `propPicker`'s
  placeholder and the rail's empty Target row both use it. Reported from a phone with the
  reference beside it, and the report is right: in the rail that dash sits beside a label
  saying *Priority*, and a row has no label, so three identical dashes said which cells
  were empty and nothing about which columns they were. Each field's own builder knows its
  empty form now — one writer per mark, never a second table: `priorityBadge(null)` is
  three bars with none lit (**free**, because the bars already sit at `opacity: .3` and
  `.on` is the only thing that raises them, so the empty form is what the component has
  always drawn and cannot drift from the filled one); `agentBadge(null)` is the glyph alone
  in `badge--dashed`, the stylesheet's own placeholder shape since `.adapted-badge` was
  written, because a handle is text and has no faint version of itself; `targetEl(null)` is
  the clock without a time. **The glyph had to be muted and that is the one new rule**: it
  is `--accent` wherever an agent *is* routed, which is the routing mark and meant to catch
  the eye, so measured on the dark theme the badge and its dashed border came out `--ink-3`
  and `--line` as intended while the robot inside stayed `#c17985` — the quietest cell in
  the row was the loudest thing in it.
- **The other half of that report is a different problem.** The reference names a column
  header too, and a header is not about empties: a *filled* priority cell is equally
  unnamed — three bars, an agent chip, an owner avatar say nothing about which column they
  are in. It is a question about the whole list, it costs a permanent line on a 390px
  phone, and it would have to pin sideways like the group heading and follow
  `--list-tracks`. Its own puck (`listan-behover-kolumnrubriker`), deliberately not smuggled
  in under this one.
...
- **Many doors, one room — so every door carries the same sign.** That is the half of the
  rule above that was stated and not carried out. The tray's rows are per-column repairs
  when a *term* hid the column: `unhideColumn` mends that term and nothing else, so
  `Show Later again` is a promise the eye keeps exactly. The archive has no per-column
  lever at all, and the row over its eye borrowed the query row's sentence anyway.
  Reported from the board: the tray offered `Done 142` and `Cancelled 2`, and pressing
  Done put both on it. The report's second half is the proof rather than a second fault —
  `⋯ → Hide column` on Cancelled writes `-status:cancelled` and hides only that, so the
  column menu keeps its word while the eye beside it does not. Not status-only either:
  two repos entirely in the archive give `Beta 3` and `Landed 1`, the eye on Beta brings
  both, **and** the column already standing grows 5 → 7, because that is what the switch
  does. Counted on the live board under `group=repo`, **seven** controls press it — one
  tray row and six column marks.
  **The lever is right and the sign was wrong.** `board.test.mjs` has asserted since it
  was written that the eye *"lifts the switch, not just its own column"*, so the
  behaviour was known, deliberate and tested; nothing on screen said so. The archive's
  rows now say `Done 142 archived`, in `archivedMark`'s own words — *"Word included, not
  a bare number"*, which that mark has carried since it was written and the tray was the
  one place not speaking it — and their tooltip promises the category (`Show archived
  pucks`) where a query row promises its column. A second spelling would be the same door
  growing a second sign, so a check asks the tray and the head for their word and
  compares them; sabotaging *either* side fells it.
  **What was deliberately not done:** making the eye write `-status:cancelled` for you.
  It was measured and it does not hold — the column already standing still grows, the
  query gains a term nobody asked for, and under `group=target` there is no term to write
  at all (`columnTerm` answers null for a month). A repair that keeps a promise in one
  grouping and cannot be expressed in another is not the promise being kept.
